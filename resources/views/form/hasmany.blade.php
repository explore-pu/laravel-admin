...
<div id="has-many-{{$column}}" class="has-many-{{$column}} form-group pl-5" {!! admin_attrs($group_attrs) !!}>
            ...
            <div class="form-group row">
                ...
                <div class="{{$viewClass['field']}}">
                    <div class="remove btn btn-warning btn-sm float-right">
                        <i class="fa fa-trash">&nbsp;</i>{{ trans('admin.remove') }}
                    </div>
                </div>
            </div>
            @endif
    ...
    <template class="{{$column}}-tpl">
        <div class="has-many-{{$column}}-form fields-group" data-key="new_{{ \Elegance\Admin\Form\NestedForm::DEFAULT_KEY_NAME }}">
...
<script>
    ...
    $('#has-many-{{ $column }}').off('click', '.add').on('click', '.add', function () {
        ...
        index++;
        var template = tpl.html().replace(/{{ \Elegance\Admin\Form\NestedForm::DEFAULT_KEY_NAME }}/g, index);
    ...
    $('#has-many-{{ $column }}').off('click', '.remove').on('click', '.remove', function () {
        ...
        $form.hide();
        $form.find('.{{ \Elegance\Admin\Form\NestedForm::REMOVE_FLAG_CLASS }}').val(1);
        return false;
    });
</script>
