...
<div id="has-many-{{$column}}" class="nav-tabs-custom has-many-{{$column}} form-group" {!! admin_attrs($group_attrs) !!}>
    ...
    <template class="nav-tab-tpl">
        <li class="nav-item" role="presentation">
            <a class="nav-link"
               id="tab-{{ ($relationName ? $relationName . '-' : '') . 'new-' . \Elegance\Admin\Form\NestedForm::DEFAULT_KEY_NAME }}"
               href="#nav-{{ ($relationName ? $relationName . '-' : '') . 'new-' . \Elegance\Admin\Form\NestedForm::DEFAULT_KEY_NAME }}"
               aria-controls="nav-{{ ($relationName ? $relationName . '-' : '') . 'new-' . \Elegance\Admin\Form\NestedForm::DEFAULT_KEY_NAME }}"
               data-toggle="tab"
               role="tab"
               aria-selected="true">
                New {{ \Elegance\Admin\Form\NestedForm::DEFAULT_KEY_NAME }}<span class="close-{{$column}}-tab text-danger d-none"><i class="fas fa-times"></i></span>
            </a>
        </li>
    </template>
    <template class="pane-tpl">
        <div class="tab-pane fields-group new"
             id="nav-{{ ($relationName ? $relationName . '-' : '') . 'new-' . \Elegance\Admin\Form\NestedForm::DEFAULT_KEY_NAME }}"
             aria-labelledby="tab-{{ ($relationName ? $relationName . '-' : '') . 'new-' . \Elegance\Admin\Form\NestedForm::DEFAULT_KEY_NAME }}"
             role="tabpanel">
            {!! $template !!}
        </div>
    </template>
...
<script>
    ...
    // 新增
        .on('click', '.add-{{$column}}-tab', function () {
            index++;
            var default_key_name = '{{ \Elegance\Admin\Form\NestedForm::DEFAULT_KEY_NAME }}';
        ...
        .on('click', '.close-{{$column}}-tab', function () {
            ...
            if (pane.hasClass('new')) {
                ...
                pane.removeClass('active').find('.{{ \Elegance\Admin\Form\NestedForm::REMOVE_FLAG_CLASS }}').val(1);
            }
