...
use Elegant\Admin\Table\Exporters\CsvExporter;
...
class Exporter
{
    ...
    public function __construct(Table $table)
    {
        ...
        $this->table->model()->usePaginate(false);
    }
    ...
    public function resolve($driver)
    {
        if ($driver instanceof Table\Exporters\AbstractExporter) {
            return $driver->setTable($this->table);
        }
    ...
    protected function getExporter($driver)
    {
        ...
        return static::$exporter = new static::$drivers[$driver]($this->table);
    }
    ...
    {
        return new CsvExporter($this->table);
    }
